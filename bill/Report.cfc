<cfcomponent displayname="report">
	...
	<cffunction name="termReport" returntype="query" access="remote">
		...
		<cfquery name="data" >
			...
			WHERE FYTotalNoOfCredits <> 0
				...
				and bc.programYear = <cfqueryparam value="#arguments.programYear#">
				and bc.term = <cfqueryparam value="#arguments.term#">
	...
	<cffunction name="attendanceReport" access="remote">
		...
		<cfquery name="data">
			select concat(bsp.lastname,', ',bsp.firstname) name
				...
				,DATE_FORMAT(IFNULL(bcStart.billingStartDate, bc.billingStartDate), '%m/%d/%Y') ReportStartDate
				,DATE_FORMAT(LAST_DAY(IFNULL(bcEnd.billingEndDate, bc.billingEndDate)), '%m/%d/%Y') ReportEndDate
				...
				bs.billingStudentId billingStudentIdMostCurrent
			FROM billingCycle bc
				<!--- this handles if billing month is split term --->
			    left outer join billingCycle bcStart on bc.StartingAttendanceBillingCycleId = bcStart.billingCycleId
			    left outer join billingCycle bcEnd on bc.EndingAttendanceBillingCycleId = bcEnd.billingCycleId
				join billingReport br on bc.LatestBillingReportID = br.billingReportId
			...
			WHERE program = <cfqueryparam value="#arguments.program#">
				...
				and (bc.billingStartDate = <cfqueryparam value="#DateFormat(arguments.billingStartDate,'yyyy-mm-dd')#">
					OR bcStart.billingStartDate = <cfqueryparam value="#DateFormat(arguments.billingStartDate,'yyyy-mm-dd')#">
					OR bcEnd.billingStartDate = <cfqueryparam value="#DateFormat(arguments.billingStartDate,'yyyy-mm-dd')#">)
	...
	<cffunction name="exitStatusReport"  returntype="query" returnformat="json"  access="remote">
		...
		<cfquery name="data">
			select bsp.LastName,
				bsp.FirstName,
				case when gender="0" then NULL else gender end gender,
				CASE WHEN ethnicity="0" then NULL else ethnicity end ethnicity,
		...
		<cfquery name="data">
			...
			from (
				select bs.contactId, bs.exitDateGroupBy exitDate
					...
					,substring_index(MAX(CONCAT(bs.billingStartDate,'|',bs.billingStudentId)),"|",-1) lastbillingStudentId
	...
	<cffunction name="termSummary" returntype="query" returnformat="json"  access="remote">
		<cfargument name="term" required="true">
		<cfquery name="data">
			select CRN, CRSE, SUBJ, Title, CASE WHEN bs.IncludeFlag=0 OR bs.IncludeFlag IS NULL THEN 'Student NOT Billed' ELSE NULL END IncludeStudent
				,bsp.bannerGNumber, bsp.firstName, bsp.lastName, bs.billingStudentId, bsi.Credits
				,schoolDistrict, bs.Program
			    ,CASE WHEN bsi.IncludeFlag=0 OR bsi.IncludeFlag IS NULL THEN 'Class NOT Billed' ELSE NULL END IncludeClass
			    ,bs.exitDate
			    ,sidnyExitDate.exitDate sidnyExitDate
		...
		<!---><cfset Session.termSummaryPrint = data>--->
		<cfreturn data>
	</cffunction>
		...
		<cfquery name="data">
			...
			where bsPrev.includeFlag = 1
				and bsPrev.billingStartDate = '#DateFormat(prevPeriod.billingStartDate, "yyyy-mm-dd")#'
			 	...
			 	and bsPrev.generatedBilledAmount != 0
			 	and (
	...
	<cffunction name="getBillingDifferencesTerm">
		...
		<cfquery name="data">
			...
			select repB.SchoolDistrict, repB.Program
			    ...
			    ,round(repB.FallCredits - ifnull(repA.FallCredits,0),1) FallCredits
			    ,round(repB.FallDays - ifnull(repA.FallDays,0),1) FallDays
			    ,round(repB.FallCreditsOverage - ifnull(repA.FallCreditsOverage,0),1) FallCreditsOverage
			    ,round(repB.FallDaysOverage - ifnull(repA.FallDaysOverage,0),1) FallDaysOverage
			    ,round(repB.WinterCredits - ifnull(repA.WinterCredits,0),1) WinterCredits
			    ,round(repB.WinterDays - ifnull(repA.WinterDays,0),1) WinterDays
			    ,round(repB.WinterCreditsOverage - ifnull(repA.WinterCreditsOverage,0),1) WinterCreditsOverage
			    ,round(repB.WinterDaysOverage - ifnull(repA.WinterDaysOverage,0),1) WinterDaysOverage
			    ,round(repB.SpringCredits - ifnull(repA.SpringCredits,0),1) SpringCredits
			    ,round(repB.SpringDays - ifnull(repA.SpringDays,0),1) SpringDays
			    ,round(repB.SpringCreditsOverage - ifnull(repA.SpringCreditsOverage,0),1) SpringCreditsOverage
			    ,round(repB.SpringDaysOverage - ifnull(repA.SpringDaysOverage,0),1) SpringDaysOverage
			...
			(select contactID, bannerGNumber, FirstName, LastName, Program, SchoolDistrict
			    ...
			    ,FYTotalNoOfCredits, FYTotalNoOfDays
			from billingCycle bc
				join billingReport br on bc.LatestBillingReportID = br.BillingReportId
			...
			select repA.SchoolDistrict, repA.Program
			    ...
			    ,round(ifnull(repB.FallCredits,0) - ifnull(repA.FallCredits,0),1) FallCredits
			    ,round(ifnull(repB.FallDays,0) - ifnull(repA.FallDays,0),1) FallDays
			    ,round(ifnull(repB.FallCreditsOverage,0) - ifnull(repA.FallCreditsOverage,0),1) FallCreditsOverage
			    ,round(ifnull(repB.FallDaysOverage,0) - ifnull(repA.FallDaysOverage,0),1) FallDaysOverage
			    ,round(ifnull(repB.WinterCredits,0) - ifnull(repA.WinterCredits,0),1) WinterCredits
			    ,round(ifnull(repB.WinterDays,0) - ifnull(repA.WinterDays,0),1) WinterDays
			    ,round(ifnull(repB.WinterCreditsOverage,0) - ifnull(repA.WinterCreditsOverage,0),1) WinterCreditsOverage
			    ,round(ifnull(repB.WinterDaysOverage,0) - ifnull(repA.WinterDaysOverage,0),1) WinterDaysOverage
			    ,round(ifnull(repB.SpringCredits,0) - ifnull(repA.SpringCredits,0),1) SpringCredits
			    ,round(ifnull(repB.SpringDays,0) - ifnull(repA.SpringDays,0),1) SpringDays
			    ,round(ifnull(repB.SpringCreditsOverage,0) - ifnull(repA.SpringCreditsOverage,0),1) SpringCreditsOverage
			    ,round(ifnull(repB.SpringDaysOverage,0) - ifnull(repA.SpringDaysOverage,0),1) SpringDaysOverage
			...
			where repB.contactID is null
				...
				<cfif maxMonth GT 23>
					...
					or IFNULL(repB.SpringDaysOverage,0) != IFNULL(repA.SpringDaysOverage,0)
				</cfif>
	...
	<cffunction name="getBillingStudentAudit" access="remote" returnformat="JSON">
		...
		<cfquery name="getAuditLogDate">
			...
			from billingCycle
			where billingType = 'attendance'
			 <!--- date last ran billing --->
			  and billingStartDate < (select max(IFNULL(bcStart.BillingStartDate, bcEnd.BillingStartDate))
										from billingCycle bcEnd
											<!--- handling if billing with month of split terms --->
											left outer join billingCycle bcStart on bcEnd.StartingAttendanceBillingCycleId = bcStart.billingCycleId
										where bcEnd.billingType = 'attendance'
										and bcEnd.billingCloseDate IS NOT NULL)
		</cfquery>

		<cfquery name="data">
			select bs.bannerGNumber, bsp.firstName, bsp.lastName, al.columnName , al.oldValue, al.newValue, ifnull(al.action, 'inserted') action
